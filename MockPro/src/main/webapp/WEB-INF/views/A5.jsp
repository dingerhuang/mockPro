...
<!-- 图书添加的模态框-->
<div class="modal fade" id="a5AddModal" tabindex="-1" role="dialog"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">A5结果添加</h4>
			</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="data" class="form-control"
								id="data_add_input" placeholder="data">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
...
<!-- 图书更新的模态框-->
<div class="modal fade" id="a5UpdateModal" tabindex="-1" role="dialog"
	aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">A5数据修改</h4>
			</div>
			<div class="modal-body">
				<form class="form-horizontal">
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="data" class="form-control"
								id="data_update_input" placeholder="data">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
				<button type="button" class="btn btn-primary" id="a5_update_btn">更新</button>
			</div>
		</div>
	</div>
</div>
...
<div class="containner">
	...
	<div class="row">
		...
		<div class="col-md-2 col-md-offset-10">
			<button class="btn btn-info glyphicon glyphicon-search" aria-label="Left Align" id="A5_query">查询</button>
		</div>
	</div>

	<div class="row">
		<div class="col-md-4">
			<button class="btn btn-primary" id="a5_add_modal_btn">新增</button>
			<button class="btn btn-danger" id="a5_delete_all">删除</button>
		</div>
	</div>

	<div class="row">
		<div class="col-md-12">
			<table class="table table-hover" id="a5_table">
				<colgroup>
				<col style="width:10%">
				<col style="width:10%">
				<col style="width:10%">
...
<script type="text/javascript">
	...
	//点击查询发送ajax请求
	$("#A5_query").click(function(){
		to_page(1);
	})
	function to_page(pn) {
		$.ajax({
			url : "${APP_PATH}/A5/lists",
			data : GetJsonData(pn),
			type : "GET",
	...
	function GetJsonData(pn) {
		...
		//alert(checkResult_query_input+"-"+priority_query_input+"-"+A5_query_input);
	...
	function build_a5_table(message) {
		//清空table 表格
		$("#a5_table tbody").empty();
		var A5S = message.extend.pageInfo.list;
		$.each(A5S, function(index, item) {
			...
			var data = $("<td></td>").append(item.data);
	...
	$(document).on("click",".delete_btn",function(){
		 ...
		 var A5Id = $(this).attr("del-id");
		 //alert($(this).parents("tr").find("td:eq(1)").text());
		 if(confirm("确定删除当前记录吗?")){
			 //确认.发送ajax请求删除
			 $.ajax({
				 url:"${APP_PATH}/A5/del/"+A5Id,
				 type:"POST",
				 success:function(message){
	...
	//新增
	$("#a5_add_modal_btn").click(function(){
		//清除表单数据(表单完整重置(表单的数据，表单的样式))
		reset_form("#a5AddModal form");
		$("#a5AddModal form")[0].reset();
		//弹出模态框
		$("#a5AddModal").modal({ //利用js创建模态框
			backdrop : "static"
		});
	...
	$("#a7_save_btn").click(function(){
		...
		$.ajax({
			url:"${APP_PATH}/A5/add",
			type:"POST",
			data:$("#a5AddModal form").serialize(),
			success:function(message){
				if(message.errorCode == 200){
					//关闭模态框
					$("#a5AddModal").modal("hide");
					to_page(99999);
				}else{
	...
	$(document).on("click",".edit_btn",function(){
		...
		//id传给模态框的更新按钮
		$("#a5_update_btn").attr("update-id",$(this).attr("edit-id"));
		//弹出模态框
		$("#a5UpdateModal").modal({
			backdrop : "static"
		});
	...
	function getA5(id){
		$.ajax({
			...
			type:"GET",
			success:function(message){
				var a5 = message.extend.a5;
				$("#priority_update_input").val(a5.priority);
				$("#errorCode_update_input").val(a5.errorCode);
				$("#errorMsg_update_input").val(a5.errorMsg);
				$("#data_update_input").val(a5.data);
				$("#timeout_update_input").val(a5.timeout);
				$("#times_update_input").val(a5.times);
			}
		});
	}
	//更新
	$("#a5_update_btn").click(function(){
		//校验post数据
		//提交请求
		var jsonData = $("#a5UpdateModal form").serializeArray();//json对象http://blog.csdn.net/csdnzhangtao5/article/details/52981541
		jsonData.push({"name":"id","value":$(this).attr("update-id")});
		//var strData = $("#a5UpdateModal form").serialize();//字符串也可以
		//strData=strData+"&"+"id="+$(this).attr("update-id");
		//可以在浏览器中直接输入$("#a5UpdateModal form").serialize();回车调试
		$.ajax({
			url:"${APP_PATH}/A5/update",
			type:"POST",
			//data:strData,
			data:jsonData,
			success:function(message){
				if(message.errorCode == 200){
					$("#a5UpdateModal").modal("hide");
					to_page(currentPage);
				}else{
					alert("更新失败");
				}
