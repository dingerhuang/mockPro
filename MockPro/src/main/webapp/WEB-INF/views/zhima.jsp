...
<!-- 图书添加的模态框-->
<div class="modal fade" id="zhimaAddModal" tabindex="-1" role="dialog"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">芝麻结果添加</h4>
			</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="result" class="form-control"
								id="result_add_input" placeholder="result">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2 control-label">authorized</label>
						<div class="col-sm-10">
							<input type="text" name="authorized" class="form-control"
								id="authorized_add_input" placeholder="authorized">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" id="zhima_save_btn">保存</button>
			</div>
		</div>
	</div>
</div>
...
<!-- 图书更新的模态框-->
<div class="modal fade" id="zhimaUpdateModal" tabindex="-1" role="dialog"
	aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">芝麻数据修改</h4>
			</div>
			<div class="modal-body">
				<form class="form-horizontal">
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="result" class="form-control"
								id="result_update_input" placeholder="result">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="authorized" class="form-control"
								id="authorized_update_input" placeholder="authorized">
								<span  class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
				<button type="button" class="btn btn-primary" id="zhima_update_btn">更新</button>
			</div>
		</div>
	</div>
</div>
...
<div class="containner">
	...
	<div class="row">
		<div class="col-md-4">
			...
			<button class="btn btn-danger" id="zhima_delete_all">删除</button>
		</div>
	</div>

	<div class="row">
		<div class="col-md-12">
			<table class="table table-hover" id="zhima_table">
				<colgroup>
				<col style="width:5%">
				<col style="width:5%">
				<col style="width:10%">
...
<script type="text/javascript">
	...
	function build_zhima_table(message) {
		//清空table 表格
		$("#zhima_table tbody").empty();
		...
		$.each(IdCardS, function(index, item) {
			...
			var result = $("<td></td>").append(item.result);
			var authorized = $("<td></td>").append(item.authorized);
	...
	//全部删除
	$("#zhima_delete_all").click(function(){
		var ids="";
	...
	$("#zhima_add_modal_btn").click(function(){
		...
		reset_form("#zhimaAddModal form");
		$("#zhimaAddModal form")[0].reset();
		//弹出模态框
		$("#zhimaAddModal").modal({ //利用js创建模态框
			backdrop : "static"
		});
	...
	$("#zhima_save_btn").click(function(){
		...
		//发送ajax请求
		$.ajax({
			url:"${APP_PATH}/Zhima/add",
			type:"POST",
			data:$("#zhimaAddModal form").serialize(),
			success:function(message){
				if(message.errorCode == 200){
					//关闭模态框
					$("#zhimaAddModal").modal("hide");
					to_page(99999);
				}else{
	...
	$(document).on("click",".edit_btn",function(){
		...
		//id传给模态框的更新按钮
		$("#zhima_update_btn").attr("update-id",$(this).attr("edit-id"));
		//弹出模态框
	...
	function getA5(id){
		$.ajax({
			url:"${APP_PATH}/Zhima/"+id,
			type:"GET",
			success:function(message){
				var zhima = message.extend.zhima;
				$("#priority_update_input").val(zhima.priority);
				$("#errorCode_update_input").val(zhima.errorCode);
				$("#errorMsg_update_input").val(zhima.errorMsg);
				$("#uid_update_input").val(zhima.uid);
				$("#result_update_input").val(zhima.result);
				$("#authorized_update_input").val(zhima.authorized);
				$("#timeout_update_input").val(zhima.timeout);
				$("#times_update_input").val(zhima.times);
			}
		});
	}
	//更新
	$("#zhima_update_btn").click(function(){
		//校验post数据
		//提交请求
